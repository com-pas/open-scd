...
This package is the local distribution/build app for the `com-pas/open-scd` monorepo. It is used for local development and testing of the OpenSCD distribution itself, and it is separate from the npm-published packages that `compas-open-scd` consumes.
...
- `pnpm run serve` starts a local server to serve the built project files (useful for testing the production version of OpenSCD)
