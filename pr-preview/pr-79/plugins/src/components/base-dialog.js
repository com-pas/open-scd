...
export class BaseDialog extends LitElement {
  ...
  render() {
    return html`<oscd-dialog @closed="${(e) => {
      ...
      <div slot="actions">
        ${this.showCancel ? html`<oscd-text-button @click="${() => this.close()}" style="--md-sys-color-primary: var(--md-sys-color-error)">
              ${get("close")}
            </oscd-text-button>` : nothing}
        ${this.renderActions()}
      </div>
    </oscd-dialog>`;
  }
