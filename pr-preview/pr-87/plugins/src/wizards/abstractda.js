...
import {
  createElement
} from "../../../_snowpack/pkg/@compas-oscd/xml.js";
import {maxLength, patterns} from "./foundation/limits.js";
import {predefinedBasicTypeEnum, valKindEnum} from "./foundation/enums.js";
function selectType(e, typeSelected, data, Val) {
  if (!e.target || !e.target.parentElement || !typeSelected)
    return;
...
function selectBType(e, bTypeSelected, bType, type) {
...
export function wizardContent(name, desc, bType, types, type, sAddr, valKind, valImport, Val, data) {
  return [
    ...
    oscdHtml`<wizard-select
      ...
      required
      @selected=${(e) => selectBType(e, predefinedBasicTypeEnum[e.detail.index], bType, type)}
    ...
    oscdHtml`<wizard-select
      ...
      helper="${get("scl.type")}"
      fixedMenuPosition
      @selected=${(e) => selectType(e, types[e.detail.index]?.id, data, Val)}
