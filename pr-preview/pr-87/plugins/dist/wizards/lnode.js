...
import { find, getValue, identity, isPublic, newWizardEvent, referencePath, newLnInstGenerator, } from '../../../_snowpack/link/packages/openscd/dist/foundation.js';
import { cloneElement, createElement, getChildElementsByTagName, } from '../../../_snowpack/pkg/@compas-oscd/xml.js';
...
function onIEDSelect(evt, parent) {
    const filteredList = evt.target;
    const lnList = getListContainer(filteredList, '#lnList');
    if (lnList === null)
        return;
    const doc = parent.ownerDocument;
    const selectedIEDItems = filteredList.selected;
    const lnItems = selectedIEDItems
        ...
        .sort(compare);
    render(html `${lnItems.map(item => html `<mwc-check-list-item
