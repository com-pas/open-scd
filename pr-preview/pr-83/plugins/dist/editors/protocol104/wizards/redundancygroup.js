...
import { getValue, newSubWizardEvent, newWizardEvent, } from '../../../../../_snowpack/pkg/@compas-oscd/open-scd/dist/foundation.js';
    ...
    return [
        {
          ...
          <oscd-list>
            ${usedLLNumbers.length != 0
                    ? usedLLNumbers.map(number => html `<oscd-list-item
                            type="button"
                            @click=${(e) => {
                        e.target.dispatchEvent(newSubWizardEvent(() => editLogicLinkWizard(parent, rGNumber, number)));
                    }}
                        >
                          Logic Link ${number}
                        </oscd-list-item>`)
