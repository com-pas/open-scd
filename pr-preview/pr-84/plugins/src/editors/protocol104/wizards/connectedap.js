      ...
      content: [
        html`<label>
          ...
          ${redundancy ? html`<h3>
                ...
                <oscd-list>
                  ${redundancyGroupNumbers.length != 0 ? redundancyGroupNumbers.map((number) => html`<oscd-list-item type="button"
                              @click=${(e) => {
          e.target.dispatchEvent(newSubWizardEvent(() => editRedundancyGroupWizard(parent, number)));
                ...
                </oscd-list>` : html`${pTypes104.map((pType) => html`${createEditTextField(parent, pType)}`)}`} `
      ]
    }
  ];
}
