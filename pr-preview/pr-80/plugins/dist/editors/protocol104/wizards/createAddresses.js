...
import { getNameAttribute, getValue, newWizardEvent, } from '../../../../../_snowpack/pkg/@compas-oscd/open-scd/dist/foundation.js';
...
function getSwitchValue(wizard, name) {
    const switchElement = wizard.shadowRoot?.querySelector(`oscd-switch[id="${name}"`);
...
export function createAddressesWizard(lnElement, doElement) {
    ...
    function renderCreateAddressesWizard() {
        ...
        function setMonitorInvertedSwitch(e) {
            const selectedTi = e.target.select.selected.value;
            const selectElement = e.target.parentElement.querySelector('oscd-switch[id="monitorInverted"]');
            if (!selectElement)
                return;
